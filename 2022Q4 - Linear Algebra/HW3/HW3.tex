...
a) Theorem 5.5(d) from the Dr. Fikus notes state that "(ii) $\mathbf{V}$ [synthesis operator] is invertible", is equivalent to "(i) ${\mathbf{v}_n}_{n\in \mathcal{N}}$ is a basis for $\mathbf{V}$". To evaluate if the given synthesis function constitutes a basis consider 
\begin{align*}
	\mathbf{V}(\mathbf{v}) = \sum_{n\in\mathcal{N}}\mathbf{x}(n)\mathbf{v}_n = 
