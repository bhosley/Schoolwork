...
\sum_{n=M+1}^{N}c_n[\mathbf{w}_n]
.\]
This combination spans \(\mathcal{V}/\mathcal{W}\) and as a subset of 
the basis of $\mathcal{V}$, is linearly independent. 
Thus there exists  a sequence \(\{\mathbf{w}_n\}_{n=M+1}^{N}\) 
that forms basis for \(\mathcal{V}/\mathcal{W}\).
Since the $M$ and $N$ where arbitrary the 
\(\operatorname{dim}(\mathcal{V}/\mathcal{W})\) 
can be determined from the basis \(\{\mathbf{w}_n\}_{n=M+1}^{N}\) as
\[
\operatorname{dim}(\mathcal{V}/\mathcal{W}) 
= N - M =
