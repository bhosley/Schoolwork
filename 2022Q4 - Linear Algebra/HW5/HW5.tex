...
(iii) The additive identity can be shown to be \([\mathbf{0}]\) by
\begin{align*}
[\mathbf{v}]+[\mathbf{0}] 
&= [\mathbf{v}+\mathbf{0}] \\
&= \{\mathbf{v}+\mathbf{0}+\mathbf{w}:\mathbf{w}\in\mathcal{W}\} \\ 
&= \{\mathbf{v}+\mathbf{w}:\mathbf{w}\in\mathcal{W}\} +
	\{\mathbf{0}+\mathbf{w}:\mathbf{w}\in\mathcal{W}\} \\ 
&= \{\mathbf{v}+\mathbf{w}:\mathbf{w}\in\mathcal{W}\} \\
&= [\mathbf{v}]
.\end{align*}

(iv)  Since $\mathcal{V}$ is a vector space there exists $-\mathbf{v}\in\mathcal{V}$.
Thus the additive inverse can be shown as
\[ 
[\mathbf{v}]+[-\mathbf{v}] =
[\mathbf{v}+(-\mathbf{v})] =
\{\mathbf{v}+(-\mathbf{v})+\mathbf{w}:\mathbf{w}\in\mathcal{W}\} =
...
\mathbf{1}[\mathbf{v}] = 
[\mathbf{1}\mathbf{v}] = 
\{\mathbf{1}\mathbf{v}+\mathbf{w}:\mathbf{w}\in\mathcal{W}\} =
\{\mathbf{v}+\mathbf{w}:\mathbf{w}\in\mathcal{W}\} =
[\mathbf{v}]
.\]
...
\[
c_1[\mathbf{v}_1+\mathbf{v}_2] =
[c_1(\mathbf{v}_1+\mathbf{v}_2)] =
[c_1\mathbf{v}_1+c_1\mathbf{v}_2] =
[c_1\mathbf{v}_1]+[c_1\mathbf{v}_2] =
c_1[\mathbf{v}_1]+c_1[\mathbf{v}_2]
.\]
...
[(c_1+c_2)\mathbf{v}] =
(c_1+c_2)[\mathbf{v}] =
c_1[\mathbf{v}]+c_2[\mathbf{v}] =
[c_1\mathbf{v}]+[c_2\mathbf{v}] =
[c_1\mathbf{v}+c_2\mathbf{v}]
.\]

Thus \(\mathcal{V}/\mathcal{W}\) is a vector space.
\end{proof}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%	\end{ Question 1 (d)}	  %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{proof}
Because \(\mathcal{W}\) is a subspace in \(\mathcal{V}\),
a basis for \(\mathcal{W}\) can be extended to be a basis in \(\mathcal{V}\).
Let \(\{\mathbf{w}_{n}\}_{n=1}^{M}\) be a basis for \(\mathcal{W}\)
and \(\{\mathbf{w}_{n}\}_{n=1}^{N}\) be a basis for \(\mathcal{V}\).
Then for each $\mathbf{v}\in\mathcal{V}$ there exists a 
$\{c_n\}_{n=1}^N$ of \(c\in\mathbb{F}\) such that,
\[
\mathbf{v} = \sum_{n=1}^{N}c_n\mathbf{w}_n = 
\sum_{n=M+1}^{N}c_n\mathbf{w}_n + \sum_{n=1}^{M}c_n\mathbf{w}_n
.\]
Additionally for each $\mathbf{w}\in\mathcal{W}$
there exists a sequence $\{b_n\}_{n=1}^N$ of \(b\in\mathbb{F}\) such that,
\[
\mathbf{w} = \sum_{n=1}^{M}b_n\mathbf{w}_n
.\]
Then for arbitrary \([\mathbf{v}]\in\mathcal{V}/\mathcal{W}\)
\[
[\mathbf{v}] = 
\sum_{n=M+1}^{N}c_n\mathbf{w}_n + 
\sum_{n=1}^{M}c_n\mathbf{w}_n + 
\sum_{n=1}^{M}b_n\mathbf{w}_n
\]
and because $\mathcal{W}$ is a subspace
\[
\sum_{n=1}^{M}c_n\mathbf{w}_n + 
\sum_{n=1}^{M}b_n\mathbf{w}_n
\in \mathcal{W}
...
\sum_{n=M+1}^{N}c_n[\mathbf{w}_n]
.\]
This combination spans \(\mathcal{V}/\mathcal{W}\) and is linearly independent. 
Thus there exists  a sequence \(\{\mathbf{w}_n\}_{n=M+1}^{N}\) 
that forms basis for \(\mathcal{V}/\mathcal{W}\).
\\

Since the $M$ and $N$ where arbitrary the \(\operatorname{dim}(mathcal{V}/\mathcal{W})\) can be determined from the basis \(\{\mathbf{w}_n\}_{n=M+1}^{N}\) as
\[
\operatorname{dim}(\mathcal{V}/\mathcal{W}) 
= N - M =
\operatorname{dim}(\mathcal{V}) - \operatorname{dim}(\mathcal{W})
.\]
\end{proof}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%	\end{ Question 1 (e)}	  %
