...
class ContactDetector(contactListener):
    # TODO: Contact needs to listen to all items
...
class LunarLander(gym.Env, EzPickle):
    ...
    Landing outside of the landing pad is possible. Fuel is infinite, so an 
    agent can learn to fly and then land on its first attempt.
    ...
    * `turbulence_power` dictates the maximum magnitude of rotational wind applied to the craft.
    ...
    - v3:
        ...
        - Fix non-deterministic behavior due to not fully destroying the world (related [GitHub issue](https://github.com/Farama-Foundation/Gymnasium/issues/728)).
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def reset(
        ...
        *,
        ...
        #    position=(1 * VIEWPORT_W / SCALE / ( 3 + 1 ), VIEWPORT_H / SCALE),
        #    angle=0.0,
        #    fixtures=fixtureDef(
        #        shape=polygonShape(
        ...
        def create_landers(num):
            landers = []
            for n in range(num):
                lander: Box2D.b2Body = self.world.CreateDynamicBody(
                    position=((n+1)/(num+1) * VIEWPORT_W / SCALE, VIEWPORT_H / SCALE),
                    angle=0.0,
                    fixtures=fixtureDef(
                        shape=polygonShape(
                            vertices=[(x / SCALE, y / SCALE) for x, y in LANDER_POLY]
                ...
                lander.color1 = ((128)%255, (102+75*(n))%255, (230)%255)
                lander.color2 = ((77)%255, (77)%255, (128)%255)
                landers.append(lander)
            return landers

        self.landers = create_landers(self.num_landers)
        self.lander = self.landers[0]
        ...
        for n,lander in enumerate(self.landers):
            for i in [-1, +1]:
                leg = self.world.CreateDynamicBody(
                    ...
                    #position=(self.lander.position[0] - i * LEG_AWAY / SCALE, self.lander.position[1]),
                    position=(lander.position[0] - i * LEG_AWAY / SCALE, lander.position[1]),
                    angle=(i * 0.05),
                    fixtures=fixtureDef(
                ...
                leg.color1 = ((128)%255, (102+75*(n))%255, (230)%255)
                leg.color2 = ((77)%255, (77)%255, (128)%255)
        ...
        self.drawlist = [self.lander] + self.legs + self.landers
    ...
    def step(self, action):
        ...
        if self.enable_wind and not any([leg.ground_contact for leg in self.legs]):
...
if __name__ == "__main__":
    env = gym.make("LunarLanders-v0", render_mode="human")
    reward = demo_heuristic_lander(env, render=True)
    print(reward)
