...
% #TODO: Add the following glossary terms to Diss.
...
While these approaches remove symmetry from homogeneous agents,
a complementary line of research exploits that symmetry directly by designing 
policies that are invariant or equivariant to permutations in their inputs. 
When agent order carries no semantic meaning, such architectures enforce this 
symmetry through shared encoders and permutation-invariant aggregations. 
These methods improve generalization and stability in settings where agents or 
observed entities are structurally uniform.

\subsection{Invariant and Equivariant Architectures}

Invariant and equivariant models operationalize symmetry by applying a shared 
encoder to each entity and aggregating the resulting features using symmetric 
operations such as summation, mean fields~\cite{yang2018, li2021b}, 
or attention~\cite{tang2021, zambaldi2018} without positional encodings. 
These designs rely on the assumption that the ordering of agents 
or entities is not semantically meaningful. 
Although not required for our formulation, 
exchangeability results 
such as de Finetti's theorem~\cite{diaconis1980} help clarify when this assumption 
is appropriate by formalizing conditions under which agents can be treated as 
statistically interchangeable~\cite{alvarez-melis2015, zaheer2017}. 
Deep Sets~\cite{zaheer2017} 
and graph-based \gls{marl} architectures~\cite{yang2021a} instantiate this principle, 
ensuring that reordering agents or entities leaves the policy unchanged. 
These methods assume a uniform input structure across agents; 
homogenization differs by aligning structurally heterogeneous observation 
spaces within a shared spanning domain, allowing invariant processing even 
when agents do not share identical feature sets.
