...
\newglossaryentry{powerset}{text=\ensuremath{\mathcal{P}},name=\ensuremath{\mathcal{P}},
    sort=P,type=symbols,category=symbol, description={Power set.}}
\newglossaryentry{naturals}{text=\ensuremath{\mathbb{N}},name=\ensuremath{\mathbb{N}},
    sort=nat,type=symbols,category=symbol, description={The set of natural numbers.}}
\newglossaryentry{integers}{text=\ensuremath{\mathbb{Z}},name=\ensuremath{\mathbb{Z}},
    sort=int,type=symbols,category=symbol, description={The set of integers.}}
...
\newglossaryentry{field}{text=\ensuremath{\mathbb{F}},name=\ensuremath{\mathbb{F}},
    sort=field,type=symbols,category=symbol, description={A field.}}
\newglossaryentry{indicator}{text=\ensuremath{\mathbb{I}},name=\ensuremath{\mathbb{I}},
    sort=field,type=symbols,category=symbol, description={Indicator Function.}}
...
\title{Implicit Indication for Shared Policies in Heterogeneous Agent Reinforcement Learning}
...
\begin{abstract}
    ...
    {\bf Objectives:} Introduce implicit indication, a representational framework for learning
    input-invariant shared policies across heterogeneous agents.\\
    {\bf Methods:} Formalize implicit indication via homogenized observation and action spaces
    with validity masks; analyze representability and invariance properties; and evaluate the
    approach empirically on arbitrary dimensional gridworld variants using MAPPO control.\\
    {\bf Results:} Comparable or improved robustness under sensor degradation and 
    team-size variation; an \(n\)-fold reduction in model storage relative to per-type baselines, 
    where \(n\) scales with team size; no measurable difference in runtime or memory usage; 
    limitations when coherence assumptions fail.\\
    {\bf Conclusions:} Implicit indication provides a practical alternative to parameter sharing
    when a coherent spanning space exists; we outline conditions, trade-offs, and open questions.
\end{abstract}
...
\citet{terry2020} evaluated encodings such as geometric masks, 
binary indicators, and inversion-based transforms that are all 
functionally equivalent to providing a one-hot identity signal.
...
To map observations into the homogenized observation space we use a zero padding. 
Define \(E_{C'} : O_{C'} \to \Hom{O}\) for each \(C' \in \mathcal{P}(C)\) such that
\[
    % E_{C'}(O_{C'}) := 
...
The two implementations use networks of identical size.
...
A summary of performance over evaluations is presented in \cref{con2:fig:evaluations}. 
...
% embedded region \(E_{C_i}(O_{C_i}) \subseteq \Hom{O}\). Because the 
% active-element pattern is always recoverable via \(\iota(E_{C_i}(o)) = C_i\), 
...
% a single policy over \(\Hom{O}\) whose restriction to each region 
...
% \(\Hom{O}\) by activating only the coordinates associated with those elements. 
% Because no two distinct subsets \(C_i\) produce overlapping embeddings, 
% the images \(E_{C_i}(O_{C_i})\) form a disjoint family within \(\Hom{O}\). 
...
improving information sharing and communication between agents that
reflects differences in composition and information access,
