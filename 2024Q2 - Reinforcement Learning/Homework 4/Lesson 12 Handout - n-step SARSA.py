...
qinit = 0.5               # Q initialization
nm1 = 8                   # Number of steps for computing n-step returns; nm1 = 0 indicat4es 1-step return
Sintervals = 11           # Number of intervals per dimension

## Algorithm Parameters
gamma = 0.999             # Discount rate 
qrange = [-200,-100]      # Range of q-values for invironment
Z = 10                    # Number of algorithm replications
M = int(0.5e3)            # Number of Episodes
test_freq = 25            # Policy Evaluation test frequency
num_test_reps = 30        # Number of replications per test
offset = 0                # Seed offset for manual testing

# State/Action Space
num_state_dims = len(env.observation_space.low) # Continuous state space
...
Smax = Srange/Sunit
SAsize = np.append(Ssize,num_actions)     # Size of State Action Space

# Convert continuous state to discrete representation
def phi(Scont) :  
    temp = np.round((Scont-Slow)/Sunit).astype(int)
    return np.minimum(temp, Smax).astype(int)
...
def evaluate_policy_milestone_test(Q, num_reps=num_test_reps, seed_mult=1):
    ...
    for rep in range(num_reps):
        terminated = False        # initialize episode conditions
        truncated = False
