...
\begin{algorithm}
    \caption{Episodic semi-gradient n-step Sarsa}
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1] % The number tells where the line numbering should start
        \State Input: a differentiable action-value function parameterization \(\hat{q}:\mathcal{S}A ⇥ Rd ! R\)
        \State Input: a policy \(\pi\) (if estimating \(q_\pi\)) 
        \State Algorithm parameters: step size ↵>0, small ">0, a positive integer n 
        \State Initialize value-function weights w 2 Rd arbitrarily (e.g., w = 0) 
        \State All store and access operations (St, At, and Rt) can take their index mod n +1
    \end{algorithmic}
\end{algorithm}
