...
```{r, echo=T}
df <- read.xlsx('./data-ex-3-1.xlsx', sheetIndex = 1)
```
...
```{r, echo=T}
names(df) <- c("Observation", "y","x1","x2")
attach(df)
```
...
```{r, echo=T}
X <- as.matrix(cbind(c(rep(1, dim(df)[1])), df[,3], df[,4]))
Y <- as.matrix(df[,2])
print(X)
print(Y)
```
...
#print(Inverse(t(X)%*%X))
print("")
print(solve(solve(t(X)%*%X)))
print(inv(inv(t(X)%*%X)))
print(ginv(ginv(t(X)%*%X)))
#print(Inverse(t(X)%*%X))
```
