...
def create_one_step_P_matrix(
    ...
    S = []
    iterable_ACFT = range(NUM_ACFT+1)
    for x0 in iterable_ACFT:
    ...
    for i_index in iterable_cardS:
        ...
        pmf_Yn_MC_failed = [binom.pmf(y,num_acft_fly,1-PROB_PASS_POSTFLT_INSP) for y in iterable_ACFT]
        # PMF of acft inbound to front shop from back shop inspection
        num_BS3 = X_n[4]
        pmf_Yn_BS3_failed = [binom.pmf(y,num_BS3,1-PROB_PASS_POST_BACKSHOP_INSP) for y in iterable_ACFT]
        pmf_Xnpl_FS = np.convolve(pmf_Yn_MC_failed,pmf_Yn_BS3_failed)
        
        ### Compute PMF of X_npl[2], Back Shop 1
        num_FS = X_n[1]
        pmf_Xnpl_BS1 = [binom.pmf(y,num_FS,1-PROB_REPAIR_FRONTSHOP) for y in iterable_ACFT]
