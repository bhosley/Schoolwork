...
\begin{tikzpicture}[>=stealth, node distance=2cm, on grid, auto,
    ...
    \node [entry, MidnightBlue] (mix) [above=1.75cm of center,minimum width=5cm]
        {Mix};
    \node [greenEntry] (state) [above left=1.2cm and 1.35cm of mix] 
        {Global State};
    \node [entry, Sepia, fill=Sepia!25] (value) 
        [above right=1.2cm and 1.35cm of mix] {Joint Value};
    \node [greenEntry, text width=1.8cm] (obs_1) [below=of agent_1] 
        {\footnotesize Local\\[-8pt]Observation};
    \node [greenEntry, text width=1.8cm] (obs_n) [below=of agent_n] 
        {\footnotesize Local\\[-8pt]Observation};

    % Relations
    \draw [arrow] (obs_1) -- (agent_1);
    \draw [arrow] (obs_n) -- (agent_n);
    \draw [arrow] (agent_1) -- +(0,1.35);
    \draw [arrow] (agent_n) -- +(0,1.35);
