...
\end{columns}
    ...
    \note[item]{This isn't a tutorial on MDPs}
    \note[item]{
...
\end{columns}
    ...
    \note{About 90 seconds.\\
    The key word to stress is \emph{deployability}.\\
    The value of the CTDE is better updates from centralization,\\
    but no need for a centralized controller.
    }
...
\end{columns}
    ...
    \note{About 90 seconds.
    This slide sets up the \emph{problem} so the next two slides
    (CTDE and PPO) can land as \emph{solutions}.
    Walk through the three consequences quickly.
    For (1), the analogy is: ``imagine playing chess, but your
    opponent changes their strategy between every move — that is
    what each agent sees.''
    For (2), point out that a team of five agents sharing one scalar
    reward has a severe signal-to-noise problem.
    For (3), briefly note that $k^n$ is why naive centralized
    control is off the table.
    The italic line at the bottom is the bridge — you can read it
    aloud and gesture forward:
    ``The next two slides show the three tools we use to fight
...
\end{block}
    ...
    \note{About 90 seconds.
    Walk through the diagram rather than the equation.
    A bried call-out to PPO, as it is used for all of these contributions. \\
    ``PPO behaves like a standard policy gradient inside the clipping region.'' \\
    ``Outside it, the gradient is zero — the update is capped.'' \\

    The MARL punchline is the last bullet: \\
    IPPO works surprisingly well not despite PPO's conservatism but \emph{because} of it.

    That conservatism is exactly what damps the instability created by
    simultaneously updating multiple agents' policies.}
    \note[item]{It is common}
    \note[item]{It is stable}
    \note[item]{It is somewhat resilient to non-stationarity}
\end{frame}
