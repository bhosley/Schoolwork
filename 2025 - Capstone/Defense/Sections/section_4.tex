...
\begin{frame}
    ...
    Then mention the landscape briefly:
    ``The two most relevant approaches \\
    — one-hot IDs \\
    - separate policies. \\
    Each solve the problem but give up something important.
    And when we looked for work on invariant representations
    as a way to enable parameter sharing in heterogeneous teams,
    there was surprisingly little.
    So we decided to investigate.''
    The alertblock should feel like a research direction being set,
    not a conclusion being announced.}
\end{frame}
...
\begin{frame}{C2 Research Questions}
    ...
    \note{
        Mention the HAPPO baseline explicitly:
        ``HAPPO is the strongest existing method that handles intrinsic heterogeneity,
        so it is the fairest comparison.''\\
        Move on quickly; the next slides deliver the answers.
    }
\end{frame}
    ...
    \begin{column}{0.50\textwidth}
        \begin{figure}
            ...
            \caption{An example 2D hypergrid with RGB channels as observation elements.
                ...
                \label{fig:rgb_hypergrid_example2}
        \end{figure}
    \end{column}
\end{columns}
\note{About 2 minutes — this is the most important technical slide in the talk.
    Deliver it in three beats.
    \textbf{Beat 1 — The idea:} ``Take every observation channel any agent could have
    and define a shared space over all of them. Each agent fills in what it can see
    and leaves the rest as zeros. The policy reads the pattern of filled-in channels
    to figure out what kind of agent it is controlling.''
    }
\end{frame}
...
\begin{frame}[shrink=10]{Training Setup}
    ...
    \note{About 1 minute.
    ``Both methods use the same network, the same hyperparameters,
    and the same compute budget.
    The only difference is how heterogeneity is handled —
    HAPPO trains a separate policy per agent type,
    while implicit indication trains one shared policy over
    the homogenized space.''}
\end{frame}
