...
\end{itemize}
    ...
    \note{Spend about 1.5 minutes here.
    The key narrative beat: Smit et al.\ showed some promise promise.\\
    \emph{What if we add a retraining phase?} \\
    And critically, \emph{does the answer depend on the type of heterogeneity?}
    }

    \note{
        Very low resource access.
        Contemporeneous to bigger successes like OpenAI Five and AlphaStar
    }
\end{frame}
...
\begin{frame}{C1 Research Questions}
    ...
    Walk through the two-phase pipeline quickly. \\
    The critical detail to stress is that both phases use \emph{independent policy updates}:
    we are not doing parameter sharing.
    }
\end{frame}
...
\end{columns}
    ...
    \note{About 1.5 minutes. \\
    Smit only compared final performance. \\
    We care about the trade-offs between and timing;\\
    thus need a cost comparison. \\
    
    Training steps is a common metric, but does not consider cost of greater numbers of agents.\\
    Wall-clock time would be a better proxy, but we use a cluster.
    
    Agent-steps gave us a fairer cost metric.

    with 8 agents costs four times as much as a 100-step run with 2 agents.''\\
    Then transition: ``Now that we have a fair yardstick, let us see what happens.''}
    \note[item]{This further allowed us to approximate the none-agent overhead cost in training.}
\end{frame}
...
\end{columns}
    ...
    \note{About 1.5 minutes.
    Waterworld is the ``easy win'' environment — use it to build confidence
    in the method before showing the harder cases.
    Walk through the heatmap: rows are target team sizes, columns are
    pretraining durations.
    The gradient is clear — bigger teams gain more from small-team pretraining.
    Mention the diminishing returns after 60 steps: this is practically useful
    because it means you do not need long pretraining to capture most of the benefit.
    Emphasize that final performance is not sacrificed — the curriculum
    reaches the same reward, just faster.
    Transition: ``Waterworld is cooperative but loosely coupled.
    What happens when coordination is tight and failure is catastrophic?''}
\end{frame}
...
\end{columns}
\note{
    Multiwalker experienced drastically differing results. \\
    It became an issue with stability of training. \\
    Too many untrained agents caused failure too early for any to learn.
}
\end{frame}
...
\end{columns}
    ...
    \note{About 1.5 minutes.
    The narrative shift here is important: in Waterworld the benefit was speed;
    in Multiwalker the benefit is \emph{survival}. \\
    
    Explain the failure mode: when one walker falls, the payload drops and
    the episode terminates for everyone.

    Without a baseline competence,
    episodes end almost immediately and the agents never get meaningful
    gradient signal.}
\end{frame}
...
\end{columns}
    ...
    \note{About 2 minutes — this is the most nuanced results slide.
        LBF is where the method hits its wall, and it is important to explain \emph{why}.\\

    Is it because coordination limitations? \\
    And how team size creates a fundamental difficulty in training that coordination,
    particularly with the environment's built-in handling of the space.

    The observation schema result is also worth a sentence:\\
    full observability significantly outperforms both reduced schemas,
    but truncated and ally-ignorant are indistinguishable from each other.
    This means that \emph{partial} teammate information is no better than none —
    a striking finding that motivates the representational rethinking in C2.
    End on the alertblock and let it land: this is the ``villain'' moment
    in the C1 story.}
\end{frame}
...
\begin{frame}{C1 Summary \& Bridge to C2}
    ...
    How the observation space changed between situations matterdd}
    ...
    \note{
        The committee should feel that C2 is the \emph{natural next question}
        that C1 raises.
        This is the most important narrative transition in the defense.}
\end{frame}
