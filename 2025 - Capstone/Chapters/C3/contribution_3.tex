...
The \emph{architectural paradigm}, exemplified by the \gls{pic}~\cite{liu2020b}, 
...
Conversely, the \emph{representational paradigm}, exemplified by Implicit Indication,%~\cite{hosley2025}, 
...
This raises a fundamental question: \textit{which paradigm better 
supports scalable and robust heterogeneous multi-agent learning?}
...
This paper follows the controlled experimental methodology introduced 
in \cref{ch:contribution_2}
to compare architectural and representational 
approaches to heterogeneity in multi-agent reinforcement learning. 
We evaluate both methods under identical training conditions in the 
HyperGrid environment, systematically varying observation structure and 
applying post-training perturbations to assess robustness and generalization. 
This design isolates the impact of the invariance mechanism itself, 
enabling a direct empirical comparison of architectural versus 
representational solutions to heterogeneous learning.
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.95\linewidth]{Figures/c3_training_curves.png}
    \caption[Implicit Indication \& PIC Training Comparison]{
        ...
        Both methods exhibit comparable learning dynamics, with no clear differences in 
        convergence rate or asymptotic training performance.
    }
...
\begin{figure}[!ht]
    ...
    \includegraphics[width=0.95\linewidth]{Figures/eval_boxes.png}
...
% In the complete-visibility configuration, Implicit Indication achieves a 
% mean return of $9.10$, compared to $5.22$ for the heterogeneous baseline 
% and $3.89$ for \gls{pic}. This performance advantage persists across all 
% sensor configurations: in intersecting-span ($5.04$ vs. $1.61$), 
...
\begin{table}[ht]
    ...
    \caption[Aggregate performance of training with different observation configurations.]{
        ...
        Values represent 95\% confidence interval for mean episode return 
    ...
    \begin{tabular}{lccc}
        ...
        Complete          & \([4.037, 4.091]\) & \([8.580, 9.028]\) & \([3.825, 4.266]\) \\
        Intersecting Span & \([4.652, 4.748]\) & \([4.556, 4.876]\) & \([1.346, 1.519]\) \\
        Disjoint Span     & \([4.288, 4.409]\) & \([5.470, 5.837]\) & \([1.802, 2.004]\) \\
        Incomplete        & \([4.093, 4.194]\) & \([4.320, 4.643]\) & \([1.150, 1.285]\) \\
        \bottomrule
    \end{tabular}
\end{table}
...
Across all evaluation types—baseline, sensor perturbations, coverage changes, 
and zero-shot generalization—Implicit Indication maintains a substantial 
performance margin over \gls{pic}, typically achieving two to four times 
higher mean returns. This consistency suggests that the performance 
difference reflects a fundamental advantage of the representational 
approach rather than sensitivity to particular evaluation conditions.

The novel-span evaluation is particularly informative, as it tests 
zero-shot generalization to observation-element combinations not 
encountered during training. Implicit Indication's strong performance 
under this condition indicates that the homogenized representation 
captures transferable structure, enabling effective behavior even 
when agent configurations differ from training.

\subsubsection{Robustness Analysis}

The evaluation conditions probing robustness—agent loss, sensor degradation, 
coverage reduction, and shuffled assignments—reveal that Implicit Indication 
degrades gracefully under structural perturbations while \gls{pic} shows 
more pronounced sensitivity. This pattern is visible in the difference 
heatmap (\cref{fig:eval_heatmaps}, right), where Implicit Indication's 
advantage is consistent across perturbation types.

The explicit masking mechanism in Implicit Indication appears to provide 
stable and interpretable encoding of observation availability, enabling 
the policy to adapt effectively when the observation structure changes 
at evaluation time. The performance gap between methods is particularly 
pronounced in configurations with inherent observation heterogeneity 
(intersecting-span and incomplete), where the representational approach's 
explicit structure may provide clearer learning signals.
