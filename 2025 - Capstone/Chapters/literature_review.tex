...
\begin{itemize}
    ...
    \item \(\gls{rew}: \Gls{s} \times \Gls{a} \rightarrow \gls{reals}\) is the reward 
        function and written as \(\gls{rew}(\gls{s}, \gls{a})\) defines the immediate reward 
...
A policy \(\gls{pi}: {\Gls{s}} \rightarrow \Delta({\Gls{a}})\) maps 
...
The agent's objective is to find an optimal policy \(\gls{pi_opt}\) that maximizes 
the expected discounted return:
\begin{equation}
    \gls{pi_opt} = \arg\max_{\gls{pi}} \gls{expRet}_{\gls{pi}} \left[ \sum_{\gls{t}=0}^{\infty} 
...
The state value function \(\Gls{v_pi}\) represents the expected return 
starting from state \(\gls{s}\) and following policy \(\gls{pi}\):
\begin{equation}
    \Gls{v_pi} = \gls{expRet}_{\gls{pi}} \left[ \sum_{\gls{t}=0}^{\infty} 
        \gls{discount}^{\gls{t}} \Gls{r}(\gls{s}_{\gls{t}}, \gls{a}_{\gls{t}}) 
        \mid \gls{s}_0 = \gls{s} \right]
\end{equation}

Similarly, the action-value function \(\Gls{q_pi}(\gls{s},\gls{a})\) captures 
the expected return of taking action $a$ in state $s$ and subsequently following \(\gls{pi}\):
\begin{equation}
    \Gls{q_pi}(\gls{s}, \gls{a}) = \gls{expRet}_{\gls{pi}} \left[ \sum_{\gls{t}=0}^{\infty}
        \gls{discount}^{\gls{t}} \Gls{r}(\gls{s}_{\gls{t}}, \gls{a}_{\gls{t}}) 
...
\(\Gls{v_*}\) and \(\Gls{q_*}\), from which optimal policies can be derived \cite{sutton2018}.

Q-learning, introduced by \cite{watkins1992}, provides a model-free method for learning 
\(\Gls{q_*}\) directly from experience without requiring knowledge of the transition dynamics. 
...
\begin{equation}
    ...
    \Gls{q}(\gls{s}_{\gls{t}}, \gls{a}_{\gls{t}}) + \gls{step-size} \left[ \gls{r}_{\gls{t}} 
        + \gls{discount} \max_{\gls{a}'} \Gls{q}(\gls{s}_{\gls{t}+1}, \gls{a}') 
        - \Gls{q}(\gls{s}_{\gls{t}}, \gls{a}_{\gls{t}}) \right]
\end{equation}
where \(\gls{step-size}\) is the learning rate. Under appropriate conditions on the learning rate 
...
maintaining decentralized actors, while \gls{happo}, extends 
this framework to heterogeneous agent settings \cite{zhong2024}. 
These extensions and their relationship to the contributions of this 
dissertation are examined in detail in \cref{subsec:mappo-variants}.
...
\begin{figure}
    \centering
    \input{hae_cycle}
    \caption{Multi-agent-environment cycle.}
    \label{fig:mae_cycle}
\end{figure}
...
\Gls{marl} extends the single-agent framework to accommodate these 
multi-agent settings (\cref{fig:mae_cycle}), introducing new challenges 
related to non-stationarity, coordination, and scalability. This section 
formalizes the \gls{marl} problem, introduces the dominant training paradigms, 
and surveys the key algorithmic approaches that underpin the contributions 
...
The \gls{posg}, which generalizes the Markov game framework
introduced by~\cite{littman1994} to settings with partial observability,
captures this extension through the components defined in \cref{tab:posg-notation}.

\begin{table}[htbp]
\centering
\caption{Components of a Partially Observable Stochastic Game.}
\label{tab:posg-notation}
\begin{tabular}{@{}cl@{}}
\toprule
\textbf{Symbol} & \textbf{Description} \\
\midrule
\(\Gls{i} = \{1, \ldots, n\}\) & Set of $n$ agents \\
\(\Gls{s}\) & Global state space \\
\(\Gls{a}_{\gls{i}}\) & Action space for agent \(\gls{i}\) \\
\(\Gls{o}_{\gls{i}}\) & Observation space for agent \(\gls{i}\) \\
\(\gls{P}: \Gls{s} \times \Gls{a} \times \Gls{s} \rightarrow [0,1]\)
    & Transition probability over joint action \(\gls{a} \in \prod_{i} \Gls{a}_{\gls{i}}\) \\
\(\Gls{r}_{\gls{i}}: \Gls{s} \times \Gls{a} \times \Gls{s} \rightarrow \gls{reals}\)
    & Reward function for agent \(\gls{i}\) \\
\(Z_i: \Gls{s} \times \Gls{a} \times \Gls{o}_{\gls{i}} \rightarrow [0,1]\)
    & Observation function for agent \(\gls{i}\) \\
\(\gls{discount} \in [0,1)\) & Discount factor \\
\bottomrule
\end{tabular}
\end{table}

When all agents share a common reward function
(\(\Gls{r}_1 = \Gls{r}_2 = \cdots = \Gls{r}_n = \Gls{r}\)), the 
\gls{posg} reduces to a \gls{dec-pomdp}, the canonical formalism 
for fully cooperative multi-agent problems~\cite{busoniu2008}. 
This dissertation focuses exclusively on cooperative settings 
formalized as \glspl{dec-pomdp}.

The joint policy \(\gls{pi} = (\gls{pi}_1, \ldots, \gls{pi}_n)\) 
induces a probability distribution over trajectories, 
and the objective is to maximize the expected discounted return:
\begin{equation}
    J(\gls{pi}) 
    = \gls{expRet}\left[
        \sum_{\gls{t}=0}^{\infty} \gls{discount}^{t} 
        \Gls{r}(\gls{s}_{t}, \gls{a}_t, \gls{s}_{t+1} \mid \gls{pi})
    \right]
...
This non-stationarity violates the assumptions that guarantee
convergence in single-agent settings, making multi-agent
learning substantially more challenging.

A further challenge arises from the exponential growth of the joint
action space. For \(n\) agents each with \(|\Gls{a}_{\gls{i}}| = k\) actions,
the joint action space has cardinality \(k^n\). This combinatorial explosion
renders naive approaches intractable even for moderate team sizes,
motivating the scalable training methods explored in this dissertation.
...
board games that motivated early game-theoretic \gls{rl} \cite{littman1994}. 
\textit{Mixed-motive} environments involve elements of both cooperation and 
competition, requiring agents to balance individual and collective interests.

Shoham et al.~\cite{shoham2007} argue that clearly specifying the multi-agent 
...
\begin{figure}
    ...
    \caption{Actor-Critic \glsxtrshort{ctde}.}
    \label{fig:ctde_actor-critic}
\end{figure}

The actor-critic architecture provides a natural implementation of \gls{ctde}: 
for example, \cref{fig:ctde_actor-critic} illustrates the relationship in which
...
Building on \gls{ppo}'s properties discussed in \cref{sec:ppo},
sever multi-agent variants have emerged to address specific 
challenges in cooperative settings.
...
reducing the number of trainable parameters \cite{yu2022}.
...
\subsubsection{HAPPO}
\label{subsubsec:happo}

\Gls{happo}~\cite{zhong2024}, addresses the challenge of 
...
\Cref{ch:contribution_1} employs \gls{ppo} within a curriculum learning framework, 
\Cref{ch:contribution_2} extends parameter sharing to heterogeneous settings 
through implicit indication, and 
\Cref{ch:contribution_3} compares architectural versus representational 
...
which are detailed in \Cref{sec:research-gaps}.
...
as demonstrated by Yu et al.~\cite{yu2022} with \gls{mappo}.
...
Despite these advantages, full parameter sharing fails 
when agents require fundamentally different behaviors. 
In \cref{subsec:taxonomy-heterogeneity} we approach the different 
aspects that condition or result from heterogeneity in more detail.
This limitation motivates the development of selective and 
...
implicit indication approach developed in \cref{ch:contribution_2}.

\subsubsection{Trade-offs: Efficiency versus Specialization}
\label{subsubsec:trade-offs_eff_v_spec}
...
while \Cref{ch:contribution_3} compares this representational approach against architectural 
...
The StarCraft Multi-Agent Challenge (SMAC)~\cite{samvelyan2019} established a 
widely-adopted benchmark based on micromanagement scenarios from StarCraft II. 
However, Ellis et al.~\cite{ellis2023} identified limitations including 
exploitable opponent behaviors, motivating SMACv2 with procedurally generated maps. 
\Gls{grf}~\cite{kurach2020} provides another influential benchmark, 
though its computational cost limits accessibility.
...
(\Cref{ch:contribution_1}), parameter sharing approaches (\Cref{ch:contribution_2}), 
and paradigms for handling heterogeneity (\Cref{ch:contribution_3}).
...
\begin{figure}
    \centering
    \input{hae_cycle}
    \caption{Heterogeneous-agent-environment cycle.}
    \label{fig:hae_cycle}
\end{figure}
...
posed by teams of structurally or behaviorally distinct agents (illustrated in 
\cref{fig:hae_cycle}). Understanding heterogeneity is central to this dissertation, 
as training shared policies across non-identical agents represents 
a core challenge addressed in \Cref{sec:research-gaps}.
...
question arises: \emph{how should agents be distinguished from one another?} 
...
\cref{ch:contribution_2} of this dissertation, which achieves agent indication 
...
in \gls{marl} broadly remains the same as \gls{marl} in general;
recalling the approaches of \cref{subsec:parameter-sharing}
(independent and shared policy networks), we consider the implications 
of a specifically heterogeneous set of agents in terms of computational 
cost, sample efficiency, and representational flexibility.
...
and represents a mid-point between a fully heterogeneous approach and
the homogenization approach developed for \cref{ch:contribution_2}.

The trade-offs between separate and shared policies, 
discussed in \cref{subsubsec:trade-offs_eff_v_spec} were not merely 
computational but also related to generalization and robustness. 
\Cref{ch:contribution_2} empirically examines these trade-offs, 
...
The architectural comparison in \cref{ch:contribution_3} further 
...
The \gls{lbf}~(\cref{subsubsec:environments}) environment illustrates 
these curriculum limitations concretely. As team sizes increase, 
the combinatorial complexity of matching agent capabilities to food 
requirements grows substantially. Policies learned with smaller 
teams may fail to develop the coordination patterns necessary 
...
\begin{figure}
    \centering
    \includegraphics[width=0.4\linewidth]{lee2019_set_transformer.png}
    \caption[Set-transformer model]{
        Set-transformer model from Lee et al.~\cite{lee2019}.}
    \label{fig:transformer}
    % #OPT: TiKZ this Diagram
\end{figure}
...
data. The Set Transformer (\cref{fig:transformer}) \cite{lee2019} 
demonstrated that attention-based architectures could achieve permutation 
invariance while capturing complex interactions between set elements,
...
\begin{figure}
    \centering
    \includegraphics[width=0.45\linewidth]{hao2022_network_overview.png}
    \caption[Hypernetwork architecture overview]{
        Hypernetwork architecture overview Hao et al.~\cite{hao2022}.}
...
domains with complex interaction structures. The GAT-MF (shown in \cref{fig:hypernetwork})
...
Li et al. \cite{li2021b} developed \gls{pipo}, which applies 
permutation-invariant representations specifically to policy learning. 
Their approach demonstrates that representational invariance 
can be achieved in the actor as well as the critic, 
enabling fully permutation-invariant multi-agent policies.
...
The implicit indication approach \cref{subsec:agent-indication}
addresses this through its homogenization of the input spaces,
without explicit agent identifiers.
...
comparison undertaken in \cref{ch:contribution_3} of this dissertation.
...
This gap motivates the research presented in \cref{ch:contribution_3} 
...
\textbf{\Cref{ch:contribution_1}} addresses this gap by evaluating curriculum-based 
...
As discussed in \cref{subsec:mappo-variants}, the \gls{happo} 
framework \cite{zhong2024} addresses intrinsic heterogeneity, 
in a manner that ensures theoretical convergence guarantees 
through sequential policy updates. However, this approach 
incurs storage costs proportional to the number of agent 
types and foregoes the data efficiency benefits of experience 
sharing across structurally different agents.
...
\textbf{\Cref{ch:contribution_2}} addresses this gap by 
introducing \emph{implicit indication}~(\cref{subsec:agent-indication}). 
...
\textbf{\Cref{ch:contribution_3}} addresses this gap through direct empirical 
...
Together, the research gaps identified in this chapter motivate a 
coherent program of three contributions addressing barriers to 
efficient and flexible training of \gls{harl} systems. The following 
chapters develop each contribution in turn, beginning with 
curriculum-based approaches to team scaling.

The contributions exhibit deliberate interconnections. 
\Cref{ch:contribution_1} identifies observation space dimensionality 
as a barrier to policy transfer across team sizes;
a limitation that \cref{ch:contribution_2} addresses through homogenization. 
...
\emph{how} agents are encoded during training. \Cref{ch:contribution_3} directly 
evaluates the homogenization approach developed in \cref{ch:contribution_2} 
against the leading architectural alternative.

All three contributions operate within the \gls{ctde} 
paradigm and employ \gls{ppo} as the base algorithm, 
enabling meaningful comparison across contributions.
The following chapters present each contribution in detail,
beginning with curriculum-based team scaling in \Cref{ch:contribution_1}.
