...
% #TODO: Summary of current state of the art and limitations with that approach (~1 paragraph)
% #TODO: summary of findings/results -- missing

% EXAMPLE
% wrt my comments on the introduction consider this:
% 1 Introduction
% According to the Global Wind Report 2023 [1], 117GW of new wind power installations
% were created in 2023, thus increasing the global cumulative wind power capacity to
% more than 1TW (a 13% increase compared to the previous year). In addition, the wind
% industry must roughly triple its annual growth to at least 320 GW by 2030 to meet the
% 28th annual United Nations Climate Change Conference (COP28) targets. However,
% as the demand for wind turbines continues to grow, it becomes crucial to have a reliable
% and cost-effective condition monitoring system to enhance their availability [2].

Aside from the afore mentioned differences in agent architecture, HARL also encompasses situations we refer to as 
\emph{behavioral heterogeneity}, % a subtype of emergent heterogeneity
% #TODO: Need to include intrinsic heterogeneity too. (Here or previous paragraph)
...
To evaluate the generalizability of this approach, 
...
direct training of policies at the full complement of target agents, 
with a strategy where policies are trained with fewer then the target number of agents 
and agent policies are then selected via bootstrapping for retraining in the target configuration.
...
% #TODO: The next paragraph may be better suited for methodology section
...
% #TODO Add paragraph summarizing novel contribution
...
Prior research has explored MARL scalability and efficiency in 
large-scale environments, investigating methods to reduce overestimation bias~\cite{ackermann2019}, 
support policy decentralization~\cite{foerster2017,lowe2020}, and optimize communication overheads 
~\cite{sukhbaatar2016,wei2022}.
...
the research from smit et al.~\cite{smit2023}
...
Prior work on policy transferability has explored architectures that support specialization and flexibility. 
...
% #TODO "I do not understand this" -Dr Cox about next paragraph
...
% #TODO: From Dr. Cox:
% Split your explanation into a base case and then specialize that base case. In other words first have a paragraph and an flowchart that shows the overall process.  
% 1) train small set of agents
% 2) bootsrtao sample from those agents
% 3) retrain for full size
% then you should have three separate (one for each environment) description of what special things you needed to do for that environment to make your basic methodology work.
