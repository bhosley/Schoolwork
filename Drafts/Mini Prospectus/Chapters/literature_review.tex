...
In a similar success, an OpenAI research team trained a group of agents 
capable of world-class performance in the game Dota 2~\cite{berner2019}.
Dota 2 bears some similarities to an \gls*{rts}; however, instead 
of managing several units, the player controls a single unit. 
The trade-off is that the single unit is more complex than those 
featured in a typical \gls{rts}, and they are drawn from a pool that is 
``massive and limitlessly diverse''~\cite{zotero-2643}.

This complexity makes it difficult to compare the action space 
between Dota 2 and StarCraft II. Additionally, the game is played 
between two teams of five players. Thus, it would be necessary to 
train an agent that could manage five complex units and decide which 
units to choose, or to train multiple agents to specialize in 
specific units and cooperate with different agents. 
...
The massive resources needed to accomplish this task 
bolsters the success against would-be challengers.

Much like Vinyals et al. the OpenAI team needed to develop a curriculum
that could effectively train their agents.
Consistent with standard software engineering practices 
(Gall's Law~\cite{gall1975}) they developed the training curriculum iteratively.

They contended with periodic updates to the game, which typically featured
changes that effected gameplay.

The general architecture that they used for the agents
had the appearance of modularity, but none of the components 
were directly shared between agents.

To improve training time and to recover agent effectiveness after
significant updates they utilized a set of procedures that they called 
``surgery'' which allowed offline updates to existing agents.

The surgery highlighted in their paper~\cite{berner2019}
was a \emph{Net2Net}~\cite{chen2016} knowledge transfer 
which allowed them to begin the training of an agent 
after a significant change without having to discard the
previous neural networks used prior to the change.
